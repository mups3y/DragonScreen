...
- ~~G0/G1 "attitude loop thrashes, fires opposing Dracos to ~0 net torque, wastes fuel"~~ — the "firing" was I1's frozen-warp artifact; realtime firing points dead-on (ptErr 0°). The RCS authority estimate flicker (L4 below) is real and — corrected 2026-08-30 — **DOES saturate the loop** (measured 51% pitch actuation saturation on flight 003648); fixed in Campaign 6.
...
| **L4** | **RCS authority estimate pinned at ~2 N·m** — stock `GetPotentialTorque` FLICKERS ~2 N·m for the Dracos; the geometric r×F fallback only fired when stock read `<1.0`, but the flicker sits ABOVE that → fallback almost never fired → the loop ran on ~2 N·m vs the real ~12–20 kN·m. **NOT latent — it SATURATES the loop** (⚠ correcting the old "not proven to break control"). | **MEASURED flight 003648 (RCS-on rows):** `ctrl_tq_pitch` median **1.8 N·m, 91% of ticks <100 N·m**; `act_pitch` fully saturated (|act|=1) **51%** of ticks = bang-bang chatter + wasted MMH/NTO (a big part of C2a). | **✅ FIXED — Campaign 6 (`c42ed20`, installed, UNFLOWN).** `AttitudeController.ControlTorque` now takes RCS authority = **max(stock report, per-axis geometric)** ALWAYS (no gate), in one alloc-free pass; the existing SmoothTorque EMA is the hysteresis. **Confirm flight owed** (rendezvous/deorbit): `ctrl_tq_*` ~12–20 kN·m not 2, `act_*` not pinned at ±1, less RCS fuel. |
...
| **F5** | max g **4.53 > 4.5** (crew cap). | **✅ RESOLVED-VERIFIED (Tick-3, flight `Crew-2_20260830_003648`, commits `a4875e9`+`f12bd32`).** ROOT = the MVac **RealFuels minThrottle floor 0.3854** (craftdump:2572), NOT limiter lag: RF remaps main→engine throttle `minThr + t·(1−minThr)`; ControlLaw's g-cap yielded the correct ENGINE throttle but returned it AS a main throttle → g = setpoint·1.107. Verified 3 ways (flight ratio 1.104–1.107 across 134620/144114/155116, config floor 0.3854, currentThrottle 0.8666=formula). FIX: `ControlLaw.ThrottleLimit` maps the cap back through the floor; `AscentControl.MinThrottle01` reads it. | **CONFIRMED:** S2 peak 4.53→**4.103** (held FLAT 4.10 to SECO), SECO throttle 0.611 (was 0.785); S1 peak **3.499** (fix helps S1 too); no spikes, reached orbit. **FIDELITY DONE:** raised S2GLimitG 4.1→**4.5** (real Crew Dragon peak) — cap now exact, installed. |
| — | **NOT a bug (flight 155116, Chris-confirmed):** the end-of-flight abort ("ABORTING DON'T PANIC" + DeorbitReturn + trunk jettison @16:08) was **Chris pressing ABORT manually** before quitting — NOT the autopilot. It VALIDATES: (a) the abort UI works (all 3 screens, regime-aware DeorbitReturn, nearest-safe-splashdown, trunk jettison, SUPPRESS RESPONSE); (b) the **FDIR correctly stayed OBSERVE-ONLY** — it logged `(observe) … [ABORT]` 10+× as RCS drained but never acted. No exceptions in the log. | positive validations | n/a |
...
| H1b | **Booster octaweb won't LIGHT during recovery — LIKELY ROOT = MODE-NUMBER MISMATCH BUG** (found by verifying Grok's pushback, NOT off-focus RF as I first claimed). `BoosterDescent.Guide` outputs `EngineMode=3` for the entry burn (`BoosterDescent.cs:104`) + `1` for the landing burn (`:134`), but `VehicleParts.EngineIdIsMode` (`VehicleParts.cs:48-55`) decodes **1=three, 2=centre, else=all/outer**. So entry (3→`else`) activates the **AllEngines/outer set = the engines that spent their 1 ignition at LIFTOFF** → RF refuses → `eng_ignited=0`; landing (1→three) activates the wrong set. `ModeName(3)="AllEngines"` is why the log said `→ AllEngines` for the intended 3-engine entry burn. | Mode-number contract mismatch between `BoosterDescent` (0/3/1) and `VehicleParts` (0/1/2). NOT proven to be off-focus RF (I over-concluded; ullage/throttle-write/eng_ignited are consistent with the mismatch). | **FIXED⚑ (Tick-1 green, installed, commit `5ec4d27`+fix, UNFLOWN)** — `BoosterDescent.Guide` now emits `ModeThreeEngine`(1) entry / `ModeCentreOnly`(2) landing; tests + comments aligned. **Re-fly: R1 `eng_ignited`/`ullage_stab` confirm the correct engines light. If STILL 0 → off-focus RF / ignition-count (Grok's other candidates).** ⭐ Grok-reviewed: off-focus-RF root was premature. | — |
...
| # | Issue | Notes | Status |
|---|---|---|---|
| M1 | Peak g 4.65 > 4.5 (crew) | ~~limiter lags ~0.35 g~~ — WRONG root; it was the minThrottle floor (see F5). `S2GLimitG` lowering 4.5→4.3→4.1 was a failed workaround. | **SUPERSEDED by F5 root-fix (Campaign 5).** |
...
| **N5** | ~~NAV globe mirrored + orbit polylines open.~~ **BOTH FALSE ALARMS (Grok wrong, disproven in the PNG preview, 2026-08-29).** (a) The globe is NOT mirrored — its strips assign u→screen the REVERSE of the flat map's Quad, so it needs NO swap; a trial swap put India/east on the LEFT (the preview caught it), reverted. (b) The orbit "gap" is NOT a closable ellipse: the overlay is `VesselData.GroundTrack`, a rotation-corrected track of span `min(period,3h)` — a spiral (or sub-period arc), so `(n-1)→0` would draw a spurious chord across the ~22° rotation seam. Left open (honest). | **RESOLVED — no NavPage change.** Kept: preview `DrawImage` reversed-U fix (flat-map texture was silently invisible in every preview) + a per-view u-convention regression test (`PageTest.NavTexture`). Campaign 4 = verification, not a code fix. |
