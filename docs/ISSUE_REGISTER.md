...
| **I3** | **Ignition-retry SPAM** — `"S2 (MVac) ignition — 0 engine(s) lit"` logged **1,246×** (every tick) on failed flights. | KSP.log 08:58: spam while the vehicle is already falling (7 km, fpa −72°, no ullage). | **OPEN — P0.0(c).** Back the retry off; log once per attempt window. |
...
| **L3** | **Booster landing burn never logged an ignition attempt** — the recovery ran (`focus → booster`, `grid fins deployed`, `engine mode → ThreeLanding`) but no landing-burn ignition line appears; `ignite_alt_m` computes garbage (jumps to 144 km, then echoes altitude). | KSP.log booster events; CSV `ignite_alt_m`. | **OPEN.** Hoverslam ignition-altitude computation is wrong; whether the engine could fire is unknown until I2. |
