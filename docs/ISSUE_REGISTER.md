...
| H1b | **Booster engine won't LIGHT on the non-active vessel** — the recovery FSM runs correctly but `eng_ignited=0` for the WHOLE descent (EntryBurn + LandingBurn, throttle 1.00) → ballistic fall 107→14 km → **LOST/destroyed 14:49:17**. | RealFuels **ullage** not settled on the falling/coasting non-active booster (the §8.6-predicted contingency). Possibly compounded by whether `ModuleEnginesRF.Activate()` + `s.mainThrottle` fully arm ignition on a non-active vessel. | **OPEN — the NEXT campaign (Actuator-ignition class):** settle ullage (fire the booster's aft RCS along the thrust axis) before each light, then verify `eng_ignited≥1`. flight 144114. Do NOT invent — measure the ullage/ignition state first. | — |
