...
| **F5** | max g **4.53 > 4.5** (crew cap). | **ROOT FOUND + FIXED (Campaign 5, headless-green, `a4875e9`, UNFLOWN).** NOT limiter lag: the MVac's **RealFuels minThrottle floor 0.3854** (craftdump:2572). RF remaps main→engine throttle `minThr + t·(1−minThr)`; ControlLaw's g-cap yielded the correct ENGINE throttle but returned it AS a main throttle → engine ran hotter → g = setpoint·(minThr+t·(1−minThr))/t. Verified 3 ways: flight ratio 1.104–1.107 (flights 134620/144114/155116), config floor 0.3854, currentThrottle 0.8666 = formula. FIX: `ControlLaw.ThrottleLimit` maps the cap back through the floor `(tgEng−minThr)/(1−minThr)`; `AscentControl.MinThrottle01` reads the field. | **CONFIRM FLIGHT owed:** peak ≈4.1 g, SECO throttle ≈0.65 (not 0.78) = mapping engaged. Then FIDELITY: real S2 ~4.5 g → RAISE S2GLimitG toward 4.5 (Chris decision). ⛔ do NOT lower it. |
