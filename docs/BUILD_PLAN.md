...
|---|---|---|---|
| `AutoPilot.Engaged` | AUTO SEQUENCE master lamp | ✅ **LIVE since W10, 2026-09-05** (the read-only host + `CrewProcedureOps`). *T17 no longer appears here: G9 (2026-09-05) closed it into W10, so there is no separate "bind the core" line — the MechJeb binding arrives with **T18 onward**.* | *satisfied.* A conductor that genuinely ticks and advances its gates; GO consumed on the frame it is pressed (W10's own rule: *a conductor with no controllers must not be ticked at all*). ⚠ The lamp means the CONDUCTOR is engaged, not that anything is flying — the host commands nothing (§B12.6 step (3)), and `CrewProcedureOps.ActivePhase` reports a phase only for a phase `FlightDriver.HasControllerFor` says is actually flown, so the glass never names a phase the vehicle is not in |
