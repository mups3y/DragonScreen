...
> research · **[SCREENS]** = UI side · **[HIST]** = historical/F9I-era, background only (don't build from it) · **[SUPERSEDED]** = obsolete instructions, quarantined to `archive/superseded/` (do not follow).
...
- **SUPERSEDED (quarantined → [archive/superseded/](archive/superseded/), do not follow — kept for history/evidence):** `ULTIMATE_PLAN.md`, `AUTOPILOT_REBUILD_PLAN.md`, `MASTER_FIX_PLAN.md`, `CAMPAIGN_PLAN.md`, `CAPABILITY_BUILD_BACKLOG.md`, `RETURN_FIX_PLAN.md`, `NOMINAL_END_TO_END_BUILD.md`.
