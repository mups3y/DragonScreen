...
- **[SEQUENCE_MAP.md](SEQUENCE_MAP.md)** [CURRENT, ⭐] — the mission phase sequence + all alternate paths + the ABORT DECISION MATRIX per phase (grounded in the real Crew Dragon 8-mode abort structure), and the STATE-AWARE RESUME map (how AUTO SEQUENCE works out where it is + what to do next — rendezvous vs deorbit, never re-launch). Drives `CrewProcedureOps.ResumeIndex`.
