...
>
> Established 2026-08-31 from the *Master Build Control Directive*, reconciled against the repository at HEAD `9073429`. Supersedes `ULTIMATE_PLAN.md` and `AUTOPILOT_REBUILD_PLAN.md` as the top governing plan (both quarantined to `docs/archive/superseded/`).
...
- **SUPERSEDED** — obsolete *instructions*; physically quarantined into `docs/archive/superseded/` with a banner, so repo structure itself prevents re-reading them as instruction.
