> **RECOVERED 2026-09-04 by W26** from `8b81816^` (deleted by `8b81816`). R1 verdict: **SUPERSEDED**.
...
> ⛔ **Named contradiction:** this file calls itself *"ACTIVE · AUTHORITY — the one governing build specification"*. **It is not.** `docs/BUILD_PLAN.md` + the §14.4 decision log are the single authoritative spec (C7.1). R1 §5.4 flags its *subsystem acceptance content* as the part worth mining.
...
**⚠ ACTIVE EXCEPTION — owner-approved temporary detour into Phase 9/12 (2026-08-31).** Re-flying the Phase-0–7 gate (ascent + booster) surfaced a TOP BLOCKER: the S2 ascent tumble (`DS-ASC-001/002`). It was root-caused and fixed — the ×1000 `AttitudeController.ControlTorque` units bug (`f1a0cbb`); ascent-to-orbit is now FLIGHT-PROVEN (`DS-ASC-003`). The owner then directed a **Phase-12 rendezvous fuel investigation** (`DS-ASC-003/004`), since a to-orbit ascent + a fuel-viable rendezvous are prerequisites they want proven before returning to the Phase-7 docking increment. This detour has modified flight-proven `ControlTorque` and the rendezvous FSM/ascent target (A1) and added a propellant guard (C) + recorder instrumentation — all under the normal rules (root-cause, smallest safe change, rule-V4 re-fly gates, no competing master plan per §3). **Return to Phase 0–7 (docking screen) once the rendezvous terminal-drain mechanism is proven from an instrumented re-fly and the smallest fix is reviewed.** Status/evidence live in `FLIGHT_VERIFICATION.md` + `COMPLETION_MATRIX.md`.
...
- 2026-08-31 — Recorded an **owner-approved temporary Phase-9/12 exception** to the Phase-0–7 active increment (§5): the ascent re-fly gate surfaced the S2 tumble (fixed, ascent flight-proven `DS-ASC-003`), then the owner directed a Phase-12 rendezvous fuel investigation (`DS-ASC-003/004`). No new master plan created; return to Phase 0–7 after the rendezvous terminal-drain mechanism is proven + fix reviewed.
