> **RECOVERED 2026-09-04 by W26** from `8b81816^` (deleted by `8b81816`). R1 verdict: **RECOVER-REFERENCE — ⛔ method only**.
> **REFERENCE ONLY — `docs/BUILD_PLAN.md` WINS on any conflict (C7.1).** Written before 2026-08-13; the
> plan has since moved on. Read it for method, evidence and reasoning — never as current instruction.
> ⛔ **STOCK-KSP regime** (R1 §0.1). Its METHOD and its recording schema transfer; **NO number in it does** — not an altitude, velocity, timing, drag figure, gain or margin.
