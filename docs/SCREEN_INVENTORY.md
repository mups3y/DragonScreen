...
| 5 | **Suit Leak Check (4.011 · ECLSS)** | Procedure: INITIATE/HALT, per-suit DELTA PRESSURE (live, §14.4(e) MARKED simulation off real cabin pressure) + STATUS (computed verdict: Nominal / amber Failed Low), TIME REMAINING, step 2.5, TROUBLESHOOT fail branch (live-on-failure repair-and-rerun, MARKED reconstruction), completion popup | **hi-res photos (discovery2/3)** + demo | ✅ UPGRADED 2026-09-01 to real wording (ECLSS subtitle, "SECTION 2: IN PROGRESS", "INITIATE SUIT LEAK CHECK", 2.4 "…nominal after time remaining is 0", TIME REMAINING row, real caution "critical…final 15 seconds…values") + reconstructed below-fold failure branch (Failed Low / TROUBLESHOOT / TRY ADDITIONAL TIMER / 2.5 Contact SpaceX). **UPGRADED 2026-09-02 (S31/S32, §14.4(e)):** ΔP + STATUS are no longer static — ΔP is four live per-suit readings off a simulated suit-loop pressure minus real cabin pressure, STATUS is a verdict on that sim (never a hardcoded word); TROUBLESHOOT is live only while a suit reads Failed Low and drives a marked repair-and-rerun (fresh seed, countdown restarts), not display-only. Popup = title/ECLSS/CLEAR/PROCEDURE COMPLETE/visor text (leak variant: FAILED LOW/SUIT LEAK DETECTED + repair-and-rerun copy) |
...
- **Suit Leak Check UPGRADED to the real page (2026-09-01)** from full-res frames (discovery2 = in-progress,
  ...
  reconstructed/below-fold. TROUBLESHOOT/TIMER wired live by T14. **UPDATED 2026-09-02 (S31/S32,
  §14.4(e)):** STATUS is no longer static "Nominal" — it's a computed verdict on a MARKED simulated
  suit-loop pressure (ΔP = suit loop − real cabin pressure, all four readings live off real cabin state,
  a 5% seedable leak chance per run); TROUBLESHOOT is live only when a suit reads "Failed Low" and drives
  a marked repair-and-rerun (fresh seed, countdown restarts to 5) — no longer display-only.
