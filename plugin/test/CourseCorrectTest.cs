...
//
// ⚠ W6, 2026-09-04. RESTORED from `8b81816^` (6,581 B, exactly R1 §5.3's figure). R1 §5.3 verdict
// **RECOVER-CODE**, calling this *"divert solve against a KNOWN LINEAR impact model — the decisive check"*.
// ONE mechanical change: the module no longer declares its own `ImpactError` (it uses the live one in
// `pure/BoosterDescent.cs`, which has no 2-arg constructor), so the positional constructions became
// the local `Err(d, c)` helper below. No check, tolerance, fixture number or assertion was altered.
// ⛔ This suite proves ARITHMETIC ONLY. `CourseCorrect`'s three constants are UN-CONVERGED for RSS-RO
// (§B16.8 ruling 2) — R1 §5.1 records the module as never flown — and the fixtures here are ANALYTIC
// (R1 §5.3: *"n/a (analytic)"*), invented to have a known closed-form answer. They are not flight data and
// a green run says nothing about tuning. See the module header.
using System;
using DragonScreen;
...
public static class CourseCorrectTest
{
    ...
    // Build the live `ImpactError` (pure/BoosterDescent.cs) positionally, as the deleted duplicate's
    // constructor used to. `GreatCircleM` stays 0: `CourseCorrect` reads only the two signed components.
    static ImpactError Err(double down, double cross)
    { ImpactError e = new ImpactError(); e.Valid = true; e.DownrangeM = down; e.CrossrangeM = cross; return e; }
    ...
    static ImpactError Model(double D0, double C0, double Kd1, double Kd2, double Kc1, double Kc2, double u1, double u2)
        => Err(D0 + Kd1 * u1 + Kd2 * u2, C0 + Kc1 * u1 + Kc2 * u2);
    ...
    public static int Run()
    {
        ...
        // ---- rank-deficient Jacobian (both axes push the impact the SAME direction) → refuse ----
        {
            ...
            ImpactError e0 = Err(D0, C0);
            ImpactError e1 = Err(D0 - 50, C0 + 50);
            ImpactError e2 = Err(D0 - 100, C0 + 100);   // = 2× axis1 direction
        ...
        {
            ImpactError e0 = Err(600, 0);
            ImpactError e1 = Err(600.2, 0);   // < MinSensitivityM (1 m)
            ImpactError e2 = Err(600, 40);
            DivertResult r = CourseCorrect.Solve2x2(e0, e1, e2, 1, 1, 1e9);
        ...
        // ---- clamp a single correction to MaxStep ----
        {
            ...
            ImpactError e0 = Err(D0, 0);
            ImpactError e1 = Err(D0 + Kd1 * 1, 0);
            ImpactError e2 = Err(D0, 40);
