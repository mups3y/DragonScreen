...
public static class ReturnTest
{
    ...
    public static int Run()
    {
        ...
        // ---- ABORT chute sequence: arm drogues → dwell → ARM mains (no cut; RealChute stages by altitude) ----
        ...
        Check("abort: drogues arm on the descent", a0.DeployDrogues && a0.Phase == ChutePhase.Drogue, "");
        Check("abort: no mains before the drogues are out", !a0.DeployMains, "");
        Check("abort: drogues are NEVER cut (backstop)", !a0.CutDrogues, "");
        ...
        Check("abort: within dwell → drogues still, mains not yet armed", a1.Phase == ChutePhase.Drogue && !a1.DeployMains, "");

        // dwell elapsed → ARM the mains THIS tick (not waiting for 1830 m); drogues stay out (no cut):
        ChuteCommand a2 = Chutes.SequenceAbort(ab, ChutePhase.Drogue, Chutes.AbortDrogueDwellSec + 0.1);
        Check("abort: after dwell → arm mains, keep drogues, advance to Main",
              a2.DeployMains && !a2.CutDrogues && a2.Phase == ChutePhase.Main, "");

        // low-altitude floor forces the mains-arm before the dwell is even up:
        ab.AltitudeM = 400;
        ChuteCommand a3 = Chutes.SequenceAbort(ab, ChutePhase.Drogue, 0.5);
        Check("abort: below the floor → immediate mains arm, no cut", a3.DeployMains && !a3.CutDrogues, "");
