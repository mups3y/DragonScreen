...
public static class BasePageIconTest
{
    ...
    {
        // §7: 893 + 5.1 + 37 + 4 + 12.3 + 16 + 4.8 + 5.3 = 977.5
        Near("§7 the icons start 5.1 below the shelf", BasePageIcon.IconTop, 898.1, 0.002);
        Near("§7 the labels start 4 below the icons", BasePageIcon.LabelTop, 939.1, 0.002);
        Near("§7 the selector sits 16 below the label", BasePageIcon.SelectorTop, 967.4, 0.002);
        Near("§11 the tab stack sums to 977.5", BasePageIcon.StackBottom, 977.5, 0.002);
    ...
    static void TheNineTabsAreWhereSection7PutsThem()
    {
        ...
        // §7: pitch 90.3, centres cx = 599.3 + i*90.3
        float[] cx = new float[9];
        for (int i = 0; i < 9; i++) cx[i] = 599.3f + i * 90.3f;

        for (int i = 0; i < 9; i++)
        {
            // icons 37 x 37, LEFT = cx - 18.5, top y = 898.1
            bool found = false;
            for (int j = 0; j < dl.Count && !found; j++)
            {
                DrawCmd c = dl.At(j);
                if (c.Kind != DrawKind.Image || c.AssetKey != key[i]) continue;
                found = Math.Abs(c.A - (cx[i] - 18.5f)) < 0.01f && Math.Abs(c.B - 898.1f) < 0.01f
                     && Math.Abs(c.C - 37f) < 0.002f && Math.Abs(c.D - 37f) < 0.002f;
                if (!found)
                    Check("§7 " + key[i] + " at " + (cx[i] - 18.5f) + ",898.1 37x37", false,
                          "drawn at " + c.A + "," + c.B + " " + c.C + "x" + c.D);
            }
            Check("§7 tab " + i + " (" + name[i] + ") draws " + key[i] + " at its centre", found, "");

            // labels: 12.3px, centred on cx, top y = 939.1
            bool lab = false;
            for (int j = 0; j < dl.Count && !lab; j++)
            {
                DrawCmd c = dl.At(j);
                if (c.Kind != DrawKind.Text || c.Str != name[i]) continue;
                lab = Math.Abs(c.A - cx[i]) < 0.01f && Math.Abs(c.B - 939.1f) < 0.01f
                   && Math.Abs(c.C - 12.3f) < 0.002f && c.Align == TextAlign.Centre;
                if (!lab)
                    Check("§7 label " + name[i] + " at " + cx[i] + ",939.1 12.3px centred", false,
        ...
        Check("§6 the shelf contains the label run it was sized off (554.1..1366.8)",
              BasePageIcon.ShelfLeft < 554.1f && BasePageIcon.ShelfRight > 1366.8f, "");
    ...
    static void TheSelectorMarksTabZeroAndNothingElse()
    {
        ...
        Check("§7 the selector sits under tab 0 (All): x 558.55, y 967.4, 81.5 x 4.8",
              FindRect(dl, 599.3f - 40.75f, 967.4f, 81.5f, 4.8f) >= 0, "");
    ...
    static void TheOldBarIsNotReferencedInLiveCode()
    {
        ...
        // ⭐ §7's stack is DERIVED. A literal 898.1 / 939.1 / 967.4 in live code would mean the stack
        // stopped following the shelf, which is the one thing §7 asks for by name.
        foreach (string lit in new[] { "898.1", "939.1", "967.4" })
