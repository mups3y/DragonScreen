...
// this file is COMMENT-ONLY. No gen-1 `BoosterTest.cs` exists (gen 1's equivalent was `HoverslamTest.cs`,
// against a DIFFERENT `HoverslamInputs` struct — not taken; see `pure/Hoverslam.cs`'s header).
//
// ⛔ WHAT A GREEN RUN OF THIS SUITE DOES AND DOES NOT PROVE. R1 §7.4 names this file, with
// `pure/Hoverslam.cs`, as a "constants with NO STATED REGIME" DEFECT, and the fixture below is where
// that defect actually lives — `Flight()` and `Booster()` carry the only anchors in the wave:
...
// (§B16.8, R1 §3.5).
// ⇒ These checks are PROPERTY checks — monotonicity, sign, unit-length, cap, FSM transition. They prove
// the solver's ARITHMETIC and the FSM's contract. They prove NOTHING about tuning, and the booster was
// NEVER RECOVERED in flight (R1 §4.2). Do not read green here as a validated ignition altitude.
//
// ⚠ AND THE FSM UNDER TEST IS FOUR PHASES, NOT §B16.2's FIVE — there is no BOOSTBACK state and no
// `TargetMode`, so nothing below exercises the always-entered, mode-parameterized boostback the owner
// settled on 2026-09-03. Full statement of the gap: `pure/BoosterDescent.cs`'s header; it has its own
// register line and is deliberately not closed by W3 (§B12.8: Wave C recovers a STARTING POINT).
using System;
using DragonScreen;
