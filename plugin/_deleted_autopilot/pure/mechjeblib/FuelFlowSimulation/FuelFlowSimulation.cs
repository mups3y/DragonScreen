...
/*
 * ---- PORTED VERBATIM into DragonScreen from MechJebLib/FuelFlowSimulation/FuelFlowSimulation.cs ----
 * Per docs/MECHJEBLIB_PORT.md. THE DRIVER: steps a SimVessel stage by stage, draining resources and
 * finishing a FuelStats segment per stage, producing the staged dv budget in `Segments`. Run() is the
 * AsyncJob entry point; DragonScreen calls it (synchronously at first) against a SimVessel built from
 * the KSP vessel. `#nullable enable annotations` legalises the `object? o` parameter.
 */
