...
/*
 * ---- PORTED VERBATIM into DragonScreen from MechJebLib/FuelFlowSimulation/DecouplingAnalyzer.cs ----
 * Per docs/MECHJEBLIB_PORT.md. Walks the part tree from the root and figures out which stage each part
 * is decoupled in - the staging model the whole dv budget rests on. `#nullable enable annotations`
 * legalises the `SimPart? parent` parameters and the property patterns are C# 8 (Roslyn latest).
 */
