...
/*
 * ---- PORTED VERBATIM into DragonScreen from MechJebLib/Utils/AsyncJob.cs ----
 * Per docs/MECHJEBLIB_PORT.md. FuelFlowSimulation derives from this so the staged-dv budget can be
 * run off the main thread (TryStartJob) rather than hitching the game every frame. The initial
 * DragonScreen wiring will call Run() synchronously; the async path is here, faithful, for later.
 *
 * `#nullable enable annotations` reproduces MechJebLib's own nullable build context so the `Task?`,
 * `object?` etc. annotations are legal - it turns the annotation context on WITHOUT the nullable
 * flow-analysis warnings, so nothing is hand-edited and the output stays clean.
 */
