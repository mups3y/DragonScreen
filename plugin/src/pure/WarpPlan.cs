...
//
// ---- RESTORED BY W4 (Wave D, §B12.8), 2026-09-04, from `8b81816^` — 4,068 B, byte-for-byte R1 §5.1's row.
// ⚠ IT HAS NO CALLER. The glue that applies these decisions (`TimeWarp.WarpTo`/`SetRate`) is
// `src/MissionConductor.cs`, which is NOT in the tree — it does not compile without `BoosterControl` (a file
// CLAUDE.md keeps deleted) and without `FlightDriver.CmdTransX/Y/Z`; register **W9**. Nothing warps today.
// ⚠ The four [Tunable]s below (BurnLeadS 12 s / SettleMarginS 3 s / MinWarpGapS 30 s / LookaheadTicks 3) are
// UN-CONVERGED for RSS-RO in the §B16.8 sense: they are researched defaults, their regime is recorded
// nowhere, and the warp orchestration they shape was never flown against a measured overshoot. They are
// margins, not physics — safe by construction (bigger = earlier drop-out) — but do not read them as tuned.
// ============================================================================================
using System;
