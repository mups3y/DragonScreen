...
namespace DragonScreen
{
    ...
    public static class NavFilter
    {
        ...
        // Correct with a relative-GPS position measurement. H = [1,0,0]; standard Kalman update.
        public static void UpdatePosition(ref AxisNav s, double zPosM)
        {
            if (!s.Init) { s = Init(zPosM, 0.0); return; }
            double S = s.P00 + RgpsNoiseM * RgpsNoiseM;      // innovation variance
            if (S <= 0.0) return;
