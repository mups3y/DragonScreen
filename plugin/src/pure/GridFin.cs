...
// is COMMENT-ONLY (comment-stripped diff against `8b81816^`: identical). No gen-1 copy of this file
// exists at `0d6423d` or `158eb2a^` — nothing to recover, nothing to collide with.
//
// ⛔ UN-CONVERGED FOR RSS-RO (§B16.8 ruling 2). As with `Hoverslam.cs`, this file declares NO constant:
// `AoaMaxDeg`, `GainDegPerKm` and `LeadTauS` are all INPUTS. The numbers therefore live in the caller and
// in `test/BoosterTest.cs`'s fixture (20° cap, 4 °/km, τ = 3 s), and NONE of them is attributed — the
// "~20°" in the header prose above is the file's own unsourced assertion, the research it cites was
// deleted 2026-09-01, and R1 §5.1 records this module as "RSS-RO researched, never DB-seeded / ❌ NO".
// `BOOSTER_GUIDANCE_METHOD.md` §10 puts the `altitude/100` AoA taper third in its re-convergence
// priority order and §1.1 states plainly that its `[F9I]` values are STOCK KERBIN and do not transfer.
// ⇒ the STEERING LAW is recoverable; its GAINS are not evidence. Re-converge them from a recorded
// RSS-RO flight before trusting a commanded angle (§B16.8 ruling 3).
// ============================================================================================
using System;
